...
void Player::_register_methods() {
  ...
  register_method("end_roll", &Player::end_roll);
  ...
  register_property<Player, float>("roll_speed", &Player::_ROLL_SPEED, 1.5);
}
...
void Player::_ready() {
  ...
  // Instance the sword hitbox node
  sword_hitbox = cast_to<SwordHitbox>(get_node("HitboxPivot/SwordHitbox"));
}

// _process() is called every frame
// delta is the time since the last frame (prevents speeding up when fps is high
// and slowing down when fps is low)
...
void Player::move() { _vel = move_and_slide(_vel); }
...
void Player::move_state(float delta) {
  ...
  // high and slowing down when fps is low) Get the input
  Vector2 input_vector = Vector2(0, 0);
  // Get the strengths of the left and right input and subtract them to get the
  // final horizontal input
  input_vector.x = _in->get_action_strength("ui_right") -
                   _in->get_action_strength("ui_left");
  // Get the strengths of the up and down input and subtract them to get the
  // final vertical input
  input_vector.y =
      _in->get_action_strength("ui_down") - _in->get_action_strength("ui_up");
  // Normalize the input vector to get a vector with a length of 1 - this is
  // needed to get the same speed when moving diagonally
  ...
  if (input_vector != Vector2(0, 0)) {
    ...
    // Set knockback vector to input vector
  ...
  if (_in->is_action_just_pressed("attack")) {
    _state = Attack;
  } else if (_in->is_action_just_pressed("roll")) {
    _state = Roll;
  }
}
...
void Player::roll_state(float delta) {
  _anim->set_animation_state("Roll");
  _vel = _roll_vector * _MAX_SPEED * _ROLL_SPEED;
  move();
}

void Player::end_attack() { _state = Move; }

void Player::end_roll() {
  _state = Move;
  _vel = _vel / _ROLL_SPEED;
}
